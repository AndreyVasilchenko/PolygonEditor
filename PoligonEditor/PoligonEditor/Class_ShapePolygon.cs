...
namespace PolygonEditor
{
    ...
    {
        private bool IsCreateWithMouse;     //  Флаг - в каком режиме происходит создание экземпляра(Ручной ввод или рисование мышью) - необходимо для начального парсинга
        public bool IsCreated;              //  Флаг - создание мышью завершилось
        ...
        //  Обновление визуального отображения фигуры
        public void UpdateShapeGeometry()
        {
            ...
            scb = new SolidColorBrush();                                                                    //  Зададим цвет заливки внутренней области фигуры
            scb.Color = (Color)ColorConverter.ConvertFromString(FillColor);
            shapeBase.Fill = scb;
        }
        ...
        public override void UpdateShapeGeometryByProperty(string property_name)
        {
            switch (property_name)
            {
                ...
                case "ShapeFillColor":
                    scb = new SolidColorBrush();                                                            //  Зададим цвет заливки внутренней области фигуры
                    scb.Color = _Editor.ShapeFillColor;
                    shapeBase.Fill = scb; 
                    FillColor = _Editor.ShapeFillColor.ToString();                                           
                    break;
            }
        }
        ...
        public override void SetTextCoords(ShapeParams param)
        {
            if (_List_tBox == null)                                                                 //  Если списка текстовых полей для ввода параметров нет
            {
                ...
                {
                    if (((Panel)_Editor.ParamPanels.Children[i]).Name == "PolygonPanel")            //      Если эта пенель для полигона, то
                    {
            ...
            for (int i = 0; i < param.GeometricParams.Count; i++)                                   //  Перебираем все параметры в полученном списке параметров 
            {
                ...
                else if (param.GeometricParams[i].ParamValue is Point)
                {
                    ...
                    throw new System.ApplicationException("ShapePolygone.SetTextCoords(): Unknown Type of ParamValue - the reason must be clarified by programmer!");
            }
        }
        ...
        public override void RemoveTextFieldFromParamPanelAt(int tbox_idx)
        {
            for (int i = 0; i < _Editor.ParamPanels.Children.Count; i++)                        //  Перебираем весь список панелей с текстовыми полями для задания параметров фигур
            {
                if (((Panel)_Editor.ParamPanels.Children[i]).Name == "PolygonPanel")
                {
                    int elem_idx = -1;
                    int t_box_count = 0;
                    int elem_count = 0;
        ...
        public override void RemoveLastTextFieldFromParamPanel()
        {
            ...
            {
                MessageBox.Show("A Polygon must have at least three points(vertices) !", "Attention!");
                return;
            }
            else
            {
        ...
        //--- Проверка самопересечений сторон поли-фигуры при использовании хелп-линии
        public override bool IsSelfIntersection(bool is_mess = true)
        {
            ...
            {
                if(!_Editor._isPrompt )
                    AddGeometryPoint(new Point(((Point)GeometricParams[0].ParamValue).X, ((Point)GeometricParams[0].ParamValue).Y));
